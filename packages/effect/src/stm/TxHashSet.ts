 ...
 *
 *   console.log(yield* TxHashSet.some(numbers, (n: number) => n > 3)) // true
 *   console.log(yield* TxHashSet.some(numbers, (n: number) => n > 10)) // false
 *
 *   const empty = yield* TxHashSet.empty<number>()
 *   console.log(yield* TxHashSet.some(empty, (n: number) => n > 0)) // false
 * })
 * ```
 *
 * @since 2.0.0
 ...
 *
 *   console.log(yield* TxHashSet.every(numbers, (n: number) => n % 2 === 0)) // true
 *   console.log(yield* TxHashSet.every(numbers, (n: number) => n > 5)) // false
 *
 *   const empty = yield* TxHashSet.empty<number>()
 *   console.log(yield* TxHashSet.every(empty, (n: number) => n > 0)) // true (vacuously true)
 * })
 * ```
 *
 * @since 2.0.0
 ...
 *   const doubled = yield* TxHashSet.map(numbers, (n: number) => n * 2)
 *
 ...
 *   const lengths = yield* TxHashSet.map(strings, (s: string) => s.length)
 ...
 *   const evens = yield* TxHashSet.filter(numbers, (n: number) => n % 2 === 0)
 *
 ...
 * const program = Effect.gen(function* () {
 *   const numbers = yield* TxHashSet.make(1, 2, 3, 4, 5)
 *   const sum = yield* TxHashSet.reduce(numbers, 0, (acc: number, n: number) => acc + n)
 *
 *   console.log(sum) // 15
 *
 *   const strings = yield* TxHashSet.make("a", "b", "c")
 *   const concatenated = yield* TxHashSet.reduce(strings, "", (acc: string, s: string) => acc + s)
